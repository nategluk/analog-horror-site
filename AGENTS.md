...
- В согласованном scope доводить локальные правки до результата: выполнить
  подходящие проверки и исправить вызванные правкой ошибки без повторного
  согласования. Остановиться при необходимости нового продуктового решения
  или действия за указанными границами.
...
- Статус хранит активную работу, препятствия и последнюю проверку, а не журнал
  всех этапов. Историю сохранять отдельно со ссылкой; старый QA не подтверждает
  текущее дерево или публичную версию.
...
- Общий UX/runtime-контракт сюжетных игр: `docs/GAME_STANDARD.md`; читать при
  создании игры или изменении её UX/media/persistence.
- Визуальный язык STAFF и носителей: `docs/CARRIER_AND_DEVICE_MAP.md`;
  читать при изменении соответствующей оболочки.
...
- После правок: `git diff --check`; для изменённого JS — `node --check`.
...
- После успешной целевой проверки расширять прогон только при новых изменениях
  или нерешённом риске. В итоге сообщить результат, выполненные проверки и
  оставшиеся ограничения; документационные правки не требуют запуска сайта.
