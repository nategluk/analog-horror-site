...
| Дерево | dirty: ЖИР ТВ корпус всегда виден; экран — только окно |
| Активная линия | ЖИР ТВ CRT + выдвижной пульт / local only |
| Последний этап | Вернул inset-окно `.cctv-screen` (случайно снято вместе с чёрным слоем) и `z-index: 0` у корпуса. Housing WebP больше не уходит за фон. Off = стекло ассета, on = шум внутри трубки. Console 0. |
