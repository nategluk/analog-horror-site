...
| Поле | Значение |
|---|---|
| Обновлено | 2026-09-15 14:34 CDT |
| Ветка / HEAD | `main` / `51fb157` |
| Дерево | dirty: сохраняются пользовательские Children footage/book/archive, телетекстовые изменения и `.DS_Store`; для «Правильного Пути» добавлен непрерывный маршрут на 22 секции, 20 активных WebP image assets и 8 документальных вставок; PNG/JPG-источники оставлены только вне runtime allowlist; commit / push / deploy не выполнялись |
| Активная линия | Черновая сборка «Правильного Пути» и микросюжет Елена → медики → книга для последующей ручной редакторской правки |
| Последний этап | После QA удалены лишние надписи черновика и внутреннего пользования; активные изображения книги, фон служебной шапки и STAFF-логотип переведены на WebP. |
