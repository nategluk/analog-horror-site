...
| Последний этап | Готово: фиксированный лист и размер иллюстрации; runtime заполняет доступную область текста по реальным DOM-размерам целыми предложениями, без повторения заголовка. Служебные PLATE-подписи и «часть N/N» удалены. Desktop и mobile `390×844`: второй разворот без overflow, без повторного заголовка и без разрыва предложения; Node и diff прошли. |
