...
продолжает работать локально: Auth URL, секреты, миграции и обе Edge Function
уже развёрнуты. Экран подтверждения реализован в репозитории, но появится на
публичном домене только после публикации сайта.
...
`auth/confirm.html` реализован локально. Пока изменения не опубликованы, текущий
хостинг по этому адресу продолжает возвращать главную страницу, поэтому
подключать публичную кнопку закрепления ещё нельзя.
...
Supabase автоматически предоставляет серверные и publishable keys. Их нельзя
копировать во frontend вручную.
...
- публикация `auth/confirm.html` и полный тест реального magic link;
- серверный режим `js/dossier-store.js`;
- синхронизация и разрешение конфликтов;
- интерфейс закрепления в звонке и `STAFF`.
