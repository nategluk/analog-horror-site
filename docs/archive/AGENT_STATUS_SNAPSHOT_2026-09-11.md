...
| Ветка / HEAD | `main` / `5dad256` |
| Дерево | dirty: STAFF HOME V2 combo geometry + прежний wallpaper/VHS-diff |
| Активная линия | ЖИР ТВ combo (экран + панель + тумбочка) / local only |
| Последний этап | Codex: полная резервная копия 66 изменений вне репозитория; 60 вспомогательных файлов дизайна/QA вынесены в архив. Код и оба ассета сохранены без изменений. |
| Commit / push / deploy | только по прямой просьбе пользователя |
| Следующий gate | фотографический корпус только если RGBA, фронт, без встроенных кнопок и обрезки |
...
- Резервная копия с SHA-256 manifest, HEAD и binary diff: `/Users/nateglukhov/Desktop/staff-home-session-backup-20260910-224856`. Исходные вспомогательные файлы — `archived-originals/`, полный снимок — `snapshot/`. Commit и откат не выполнялись.

- STAFF HOME V2 runtime `390×844`: POWER on/off, CH+/CH−, VOL mute/unmute (индикатор MUTE и `video.muted` на эфире и кассете), TXT смена страницы, AV каталог, VCR play `readyState 4` и EJECT возвращает каталог с сохранённой кассетой. Reduced motion: `animation: none` на шуме, кнопки без transition, SERVICE/канал/mute/TXT/AV живые. Fallback: abort MP4 оставляет poster эфира в экране `326×244.5`; кассета `is-media-fallback` + «СИГНАЛ НЕ ЧИТАЕТСЯ // ПОКАЗАН КАДР». Console без ошибок на живом маршруте. `node --check`, `git diff --check`.
...
- Archive cells pilot: STAFF `archive.html` обозначает три металлические
  архивные ячейки вместо кассет; после удаления фото-рубрики остаются 11
  материалов: 5 протоколов, 5 досье и 1 издание. Формулировка возврата из
  бумажного материала — «ВЕРНУТЬ ДОКУМЕНТ В АРХИВ».
...
- Legacy archive indexes: `documents.html` и `photos.html` сохранены в Git, но
  ...
  `archive`; `_headers` оставляет noindex для рабочих `documents/*`.
  Фотофайлы не удалялись: они остаются сюжетными вложениями звонка Ирины.
...
- Carrier/device map: добавлен `docs/CARRIER_AND_DEVICE_MAP.md`. Зафиксированы
  ...
  дела. Фото остаётся носителем `PHOTO-EVIDENCE`, но больше не является
  отдельной ячейкой production-архива.
...
- Archive counts: история добавлений показала, что guest-count обновлялся вместе
  ...
  summary. После удаления фото-рубрики актуальный STAFF-count — `5 + 5 + 1 = 11`;
  гостевой контур показывает три закрытые папки: протоколы, личные дела и издания.
