alter table public.dossiers
  drop constraint if exists dossiers_role_check;

alter table public.dossiers
  add constraint dossiers_role_check
  check (role is null or role in ('animator', 'volunteer', 'impostor'));

alter table public.dossier_sessions
  drop constraint if exists dossier_sessions_role_check;

alter table public.dossier_sessions
  add constraint dossier_sessions_role_check
  check (role is null or role in ('animator', 'volunteer', 'impostor'));
...
begin
  ...
  if jsonb_typeof(dossier_payload) <> 'object'
    ...
    or dossier_payload ->> 'role' not in ('animator', 'volunteer', 'impostor')
  then
    return jsonb_build_object('status', 'invalid');
  end if;
...
comment on function public.consume_dossier_claim(uuid, text, uuid, text) is
  'Атомарно закрепляет дело сотрудника или самозванца за auth-пользователем.';
